...
export class QADataSetStore {
  ...
  private query = debounce(async (spaceId: string) => {
    const { data = {} } = await knowledgeApi.QAListSelectDataset();
    ...
    if (unUseIds.length > 0) {
      Toast.error(
        I18n.t(
          'workflow_qa_knowledge_un_exit_error',
          {
            id: unUseIds.join(','),
          },
          '问答库不存在',
    ...
    unUseIds?.forEach(id => {
      if (!this.dataSetInfosMap[id]) {
        this.dataSetInfosMap[id] = {
          dataset_id: id,
          name: I18n.t('workflow_qa_knowledge_un_exit', {}, '问答库不存在'),
        };
      }
    });
  };
