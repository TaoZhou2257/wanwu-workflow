...
/**
 * Use the same useQADataSetInfos as the knowledge list in the form.
 * useQADataSetInfos will cache the data internally. When a new knowledge base is selected in the form, there is no need to resend the request to pull the data.
 */

import { useQADataSetInfos } from '@/hooks';

import { OverflowTagList } from './overflow-tag-list';
import { Field } from './field';

export function QAKnowledgeWanwu() {
  const { data } = useWorkflowNode();
  const knowledge = data?.inputs?.datasetParameters?.datasetParam ?? [];
  const { dataSets } = useQADataSetInfos({ ids: knowledge.map(item => item.dataset_id) });
  return (
    <Field
      label={I18n.t('workflow_detail_qa_knowledge_knowledge')}
