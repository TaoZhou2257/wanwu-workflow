...
const formatToolParams = (apiParam: any) => {
  const nameMapObj = {
    toolType: 'toolType',
    pluginID: 'toolId',
    pluginName: 'toolName',
  ...
  // array
  /*let toolParams = Object.keys(apiParam).map(key => {
    return ({...apiParam[key], name: nameMapObj[apiParam[key]?.name]})
  })*/

  // object
  let toolParams = {}
  Object.keys(apiParam).map(key => {
    toolParams[nameMapObj[apiParam[key]?.name]] = apiParam[key]?.input?.value?.content
  })
  return toolParams
}
/**
...
export const transformOnSubmit = (
  ...
  const submitValue = {
    ...
    inputs: {
      ...value?.inputs,
      toolParams: formatToolParams(value?.inputs?.apiParam || {}),
