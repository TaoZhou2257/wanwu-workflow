...
export const FormRender = ({ form }: FormRenderProps<ApiNodeFormData>) => {
  ...
  const indentifier: any = getApiNodeIdentifier(
  ...
  const setApiKey = (value) => {
    const apiParams = form.getValueIn('inputs.apiParam')
    const newApiParams = apiParams.map(item => {
      if (item.name === 'apiKey') {
        let newItem = {...item}
        newItem.input.value.content = value
        return newItem
      } else {
        return item
      }
    })
    form.setValueIn('inputs.apiParam', newApiParams)
  }
