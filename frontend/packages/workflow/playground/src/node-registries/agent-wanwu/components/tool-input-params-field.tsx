...
import { DEFAULT_PARAMS_LIST, FILE_INPUT_AVAILABLE_TYPES } from '../constants';
...
export const ToolInputParamsField = withFieldArray(() => {
  const { value, remove, append } = useFieldArray<InputValueVO>();
  const { data } = useWorkflowNode();
  const removeAll = () => {
    const valueArr = JSON.parse(JSON.stringify(Array.isArray(value) ? value : []))
    valueArr.forEach(() => {
      remove(0)
    })
  }
    ...
    >
      ...
      <FieldArrayList>
        {value?.map(({name}, index) => {
          ...
          // 限制文件类型
          const limitFileTypes = type === ViewVariableType.File;
          return (
            <ValueExpressionInputField
              ...
              required={required}
              inputType={type}
              availableFileTypes={limitFileTypes ? [...FILE_INPUT_AVAILABLE_TYPES] : undefined}
