...
export const useQADataSetInfos = ({ ids }: { ids: string[] }) => {
  const [dataSets, setDataSets] = useState<Dataset[]>([]);
  const [isReady, setReady] = useState(false);
  const { spaceId, sharedQADataSetStore } = useGlobalState();
  ...
  const getDataSetInfos = useCallback(
    async (_ids: string[]) => {
      try {
        const _dataSets = await sharedQADataSetStore.getDataSetInfosByIds(
          _ids,
          spaceId,
        );
  ...
  return {
    dataSets,
    isReady,
    cacheDataSetInfo: sharedQADataSetStore.addDataSetInfo,
  };
};
