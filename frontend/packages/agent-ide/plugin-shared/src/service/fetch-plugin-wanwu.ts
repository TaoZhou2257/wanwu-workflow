...
function formatPluginApiProductInfo(productInfo: ProductInfo) {
  const name = productInfo?.toolName;
  const pluginId = `${productInfo?.toolId || ''}`;
  const apiList = formatActions(productInfo?.actions || []);
...
const formatActions = (actions: any) => {
  return actions?.map((action: any) => ({id: action.actionId, name: action.actionName, description: action.desc}))
}
...
function formatPluginProductInfo(
  ...
  return {
    productInfo: {
      ...
      ...item.plugin_extra,*/
      is_free: true, //item?.meta_info?.is_free,
      tools: formatActions(item?.actions || [])
    },
    pluginInfo: {
      name: item?.toolName,
      plugin_apis: formatPluginApiProductInfo(item),
      id: `${item?.toolId || ''}`,
      plugin_icon: item?.iconUrl,
      plugin_type: item?.toolType,
      desc_for_human: item?.desc,
      /*creator: {
...
async function getPluginFromMarket(
  ...
  const res = await ProductApi.GetToolList(
    {
      name: search,
      toolType: type
    }
